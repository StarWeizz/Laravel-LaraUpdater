...
class UpdateManager
{
    ...
    public function hasUpdate(bool $force = false): bool
    {
        ...
        return version_compare($version, App::version(), '>');
    }
    ...
    public function forceFetchUpdates(bool $cache = true): ?array
    {
        $updates = $this->prepareHttpRequest()
            ->get('http://localhost/api/updates/index')
            ->throw()
            ->json();
    ...
    {
        $userAgent = 'App updater (v'.App::version().' - '.url('/').')';

        $request = Http::withUserAgent($userAgent)->withHeaders([
            'App-Version' => App::version(),
            'App-PHP-Version' => PHP_VERSION,
            'App-Locale' => app()->getLocale(),
            'App-Users' => User::count(),
        ]);
